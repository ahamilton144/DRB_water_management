...
class PywrDRBModel():
    def __init__(self, inflow_type, model_filename, start_date, end_date,
                 options={"inflow_ensemble_indices":None}):
        ...
        self.model_dict = {
            ...
            "timestepper": {"start": None, "end": None, "timestep": None},
    ...
    def add_timestepper(self, start_date, end_date, timestep):
        self.model_dict["timestepper"]["start"] = start_date
        self.model_dict["timestepper"]["end"] = end_date
        self.model_dict["timestepper"]["timestep"] = timestep

    def add_inflow_scenarios(self, inflow_ensemble_indices):
        # After knowing how this works, we might want to revist this method.
        self.NSCENARIOS = len(inflow_ensemble_indices)
        self.model_dict["scenarios"] = [{"name": "inflow", "size": self.NSCENARIOS}]

    def check_nodes(self, node_name, category):
        # Pending 
        if node_name in self.nodes:
            raise ValueError(
                f"Node {node_name} already exists in model. Please use a unique name."
            )
                
    def add_major_reservoir_node(self, reservoir_name, downstream_lag, capacity, variable_cost=False):
